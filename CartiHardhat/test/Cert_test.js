...
describe('CertModule',()=>{
    async function deployContract(){
        ...
        it('can read certificate',async()=>{
            const {admin,other,cert} = await loadFixture(deployContract);
             await cert.issue(123,"Achu",'CED','A','07/06/2025');
             const certificate= await cert.Certificate(123);
             console.log(certificate);
             expect(certificate[0]).to.equals('Achu');
        ...
        it('only admin can issue',async()=>{
            const {admin,other,cert}=await loadFixture(deployContract);
            await expect( cert.connect(other).issue(123,"Achu",'CED','A','07/06/2025')).to.be.revertedWith('Unauthorised Access')
        })
